...
export function renderStudio({host,initialDraft={},materials=[],resolveMaterial,onDraft=()=>{},setStatus=()=>{}}){
  ...
  const materialImage=(name,instance={})=>{
    ...
    const candidates=localAssets.filter(a=>a&&a.material_id===instance.materialId&&a.spec_id===instance.specId&&!a.needs_mask&&(!a.status||a.status==='ready')&&a.rights_status!=='prohibited'&&a.imageUrl);
    const local=[...sourceClasses,'generated_from_evidence'].map(sourceClass=>candidates.find(a=>a.representation_class===sourceClass)).find(Boolean);
    return local?{...generated,atlas:undefined,imageUrl:local.imageUrl,assetRef:local.file,provenanceClass:local.representation_class,subjectBounds:local.subject_bounds||local.subjectBounds}:generated;
  };
  const meta=m=>{const spec=specs.get(m.name)||{sizeMm:8,form:m.category==='hardware'?'connector':tab==='focal'?'irregular':'round'};const materialId=m.materialId||`candidate-${slug(m.name)}`;return {...spec,materialName:m.name,materialId,specId:`${materialId}-${spec.form}-${spec.sizeMm}mm`,displayNameZh:m.zhName,displayNameEn:m.name,sourceStatus:'PROPOSED',mappingStatus:'unverified',provenanceClass:'generated_from_evidence'};};
  ...
  const placement=m=>{const p=meta(m),img=materialImage(m.name,p);return {...p,...(img.imageUrl?{imageUrl:img.imageUrl,assetRef:img.assetRef,provenanceClass:img.provenanceClass,subjectBounds:img.subjectBounds}:img.atlas?{atlas:img.atlas,assetRef:img.assetRef,provenanceClass:img.provenanceClass}:{})};};
