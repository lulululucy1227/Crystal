...
export function renderStudio({host,initialDraft={},materials=[],resolveMaterial,onDraft=()=>{},setStatus=()=>{}}){
  ...
  canvas=createBraceletCanvas({canvasElement:q('[data-studio-canvas]'),state:history.present,resolveMaterial:materialImage,onCommand:command=>{if(disposed)return;if(command.type==='select-instance'){history.present={...history.present,selectedInstanceId:command.instanceId};sync();selection();canvas?.render(history.present);return;}if(command.type==='place'){const m=materials.find(m=>m.name===active);if(!m)return;command={type:'place',looseX:command.looseX,looseY:command.looseY,slotIndex:command.slotIndex,...meta(m)};}commit(command);}});
