...
export function createBraceletCanvas({canvasElement,state,resolveMaterial,onCommand,trashElement}){
 ...
 let currentState=state,viewOptions={},projection,disposed=false,sequence=0,frame=0,animationGeneration=0,drag=null,preview=null,visualKey='';
 const objects=new Map(),imageCache=new Map(),guides=[];
 const tray=new Circle({...inert,radius:1,fill:'#e7dfd3',stroke:'#bdb1a1',strokeWidth:2,shadow:{color:'rgba(65,49,30,0.16)',blur:16,offsetX:0,offsetY:7}});
 const rim=new Circle({...inert,radius:1,fill:'transparent',stroke:'#f5efe7',strokeWidth:2});
 const linearTray=new Rect({...inert,width:1,height:1,rx:36,ry:36,fill:'#e7dfd3',stroke:'#bdb1a1',strokeWidth:2,visible:false,shadow:{color:'rgba(65,49,30,.12)',blur:16,offsetY:7}});
 const outline=new Circle({...inert,selectionIndicator:true,visible:false,radius:0,fill:'transparent',stroke:'#637b6a',strokeWidth:1.5});
 const gapMarker=new Circle({...inert,visible:false,radius:7,fill:'#647965',stroke:'#fff',strokeWidth:2});
 canvas.add(tray);canvas.add(rim);canvas.add(linearTray);canvas.add(outline);canvas.add(gapMarker);
 ...
 function selection(object){outline.set(object&&object.visible!==false?{visible:true,left:object.left,top:object.top,radius:object.data.diameter/2+4,scaleY:object.height&&object.width?Math.min(1,object.height/object.width):1,angle:object.angle||0}:{visible:false});canvas.bringObjectToFront?.(outline);}
 ...
 function drawGuides(){
  ...
  tray.set({left:p.center.x,top:p.center.y,radius:currentState.layoutMode?p.trayRadius:p.radius,visible:round});rim.set({left:p.center.x,top:p.center.y,radius:p.trayRadius-7,visible:round&&!!currentState.layoutMode});
  linearTray.set({left:p.center.x,top:p.center.y,width:p.width*.94,height:p.height*.86,visible:!round});
  guides.forEach(o=>{canvas.remove(o);o.dispose?.();});guides.length=0;
  if(currentState.layoutMode==='bracelet')p.guides.filter(g=>g.visible!==false).forEach(g=>{
   const guide=round?new Circle({...inert,left:p.center.x,top:p.center.y,radius:g.radius,fill:'transparent',stroke:'#cfc2b0',strokeWidth:1}):new Rect({...inert,left:p.center.x,top:g.y,width:g.x2-g.x1,height:Math.max(6,p.scale*2),rx:5,ry:5,fill:'#d4c8b8',stroke:'#ede6dc',strokeWidth:1});
   const label=new FabricText(`${round?g.row+1:`${g.section==='front'?'正手':'背手'} ${g.wrapIndex}`} · ${g.startIndex+1}–${g.startIndex+g.count} ${g.direction<0?'←':'→'}`,{...inert,left:round?p.center.x:p.width*.13,top:round?p.center.y-p.trayRadius+18+g.row*17:g.y-Math.max(20,p.scale*9),fontSize:11,fontFamily:'Microsoft YaHei UI, sans-serif',fill:'#766e60'});
   guides.push(guide,label);canvas.add(guide);canvas.add(label);
  });
 ...
 async function render(nextState=currentState,options=viewOptions){
  ...
  const key=JSON.stringify([canvas.getWidth(),canvas.getHeight(),nextState.layoutMode,nextState.trayMode,nextState.wrapCount,nextState.wristCm,nextState.instances,viewOptions]);
  ...
  desired.forEach(({instance,object,isNew},index)=>{
   ...
   object.set({data:{instanceId:instance.instanceId,materialName:instance.materialName,diameter:p.diameter},hoverCursor:'grab',moveCursor:'grabbing',opacity:1,visible:p.visible!==false,evented:p.visible!==false,selectable:p.visible!==false});
 ...
 function resize(){const host=canvasElement.parentElement?.parentElement?.classList?.contains('p4-tray-stage')?canvasElement.parentElement.parentElement:canvasElement.parentElement;canvas.setDimensions({width:Math.max(280,host?.clientWidth||520),height:Math.max(300,host?.clientHeight||520)});visualKey='';return render(currentState);}
 ...
 canvas.on('mouse:down',event=>{
  ...
  // Legacy slot editor keeps click-to-place; Studio placement is explicit + or drag.
 ...
 return {render,resize,
  ...
  async exportImage(){await render(currentState);cancelFrame();const points=project(currentState,canvas.getWidth(),canvas.getHeight()).points;points.forEach(p=>{const object=objects.get(p.instanceId);if(object){object.set({left:p.x,top:p.y,scaleX:p.diameter/object.baseSize,scaleY:p.diameter/object.baseSize,angle:p.rotationDeg||0});object.setCoords();}});record();outline.set({visible:false});gapMarker.set({visible:false});canvas.discardActiveObject();canvas.renderAll();const url=canvas.toDataURL({format:'png',multiplier:2});selection(objects.get(currentState.selectedInstanceId));canvas.requestRenderAll();return url;},
