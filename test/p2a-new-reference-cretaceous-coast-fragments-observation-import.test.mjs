...
function fixture() {
  ...
  db.exec(migration9); db.exec('PRAGMA foreign_keys=ON'); db.close(); return { dir, dbPath, inputPath, close() { rmSync(dir, { recursive: true, force: true, maxRetries: 5, retryDelay: 100 }); } };
}
...
test('Coast observations import exact 13 rows and fully replay', () => {
  const f = fixture(); try {
    ...
    const db = new DatabaseSync(f.dbPath); assert.equal(db.prepare('SELECT COUNT(*) n FROM design_reference_synthesis_assertion').get().n, 0); assert.equal(db.prepare('SELECT COUNT(*) n FROM design_reference_synthesis_source').get().n, 0); db.close();
  } finally { f.close(); }
});
