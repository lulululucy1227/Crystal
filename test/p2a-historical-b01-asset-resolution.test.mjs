...
function fixture() {
  ...
  db.exec(migration9); db.exec('PRAGMA foreign_keys=ON;');
...
test('B01 resolver dry-run is read-only, then applies exactly five fixed metadata identities and replays', () => {
  const f = fixture(); try {
    ...
    const db = new DatabaseSync(f.dbPath); assert.equal(db.prepare('SELECT COUNT(*) n FROM image_visual_observation').get().n, 45); assert.equal(db.prepare('SELECT COUNT(*) n FROM design_reference_synthesis_assertion').get().n, 0); assert.equal(db.prepare('SELECT COUNT(*) n FROM design_reference_synthesis_source').get().n, 0); assert.equal(db.prepare('PRAGMA integrity_check').get().integrity_check, 'ok'); assert.deepEqual(db.prepare('PRAGMA foreign_key_check').all(), []); db.close();
  } finally { f.close(); }
});
