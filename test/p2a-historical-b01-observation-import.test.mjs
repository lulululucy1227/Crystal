...
test('B01 imports the fixed 22 rows once and then exactly reuses them', () => {
  const f = fixture(); try {
    ...
    const db = new DatabaseSync(f.dbPath); assert.equal(db.prepare('SELECT COUNT(*) n FROM image_visual_observation').get().n, 67); assert.equal(db.prepare('SELECT COUNT(*) n FROM design_reference_synthesis_assertion').get().n, 0); assert.equal(db.prepare('SELECT COUNT(*) n FROM design_reference_synthesis_source').get().n, 0); db.close();
  } finally { f.close(); }
});
