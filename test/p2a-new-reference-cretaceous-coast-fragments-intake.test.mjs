...
function fixture() {
  ...
  db.close(); return { dir, dbPath, inputPath, close() { rmSync(dir, { recursive: true, force: true, maxRetries: 5, retryDelay: 100 }); } };
}
...
test('Cretaceous Coast Fragments intake creates one mechanical reference and exactly replays it', () => {
  const f = fixture(); try {
    ...
    assert.match(first.reference_key, /^REF-\d{6}$/); assert.deepEqual(counts(f.dbPath), { refs: 1, assets: 3, links: 3, observations: 0, synthesis: 19 });
